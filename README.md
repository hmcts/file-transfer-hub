...
- Prod: Terraform does not generate the FTPS login secrets, BAIS target secrets, or certificate secrets. They must be created in Key Vault before the container-app deployment is planned or applied. When `ftps.manage_storage_sftp_target = true`, Terraform does create the Azure Storage SFTP local user for the first forward target plus the matching `ftps-storage-sftp-username` and `ftps-storage-sftp-password` secrets.
...
The optional `ftps.manage_storage_sftp_target` flag keeps the existing nonprod behavior and allows prod to opt in to Terraform-managed Azure Storage SFTP credentials for the first forward target.
